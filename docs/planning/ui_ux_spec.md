...
### 5.3. Lyrics Control Bar (Header)
- **Visibility**:
  - 가사가 로드되지 않은 상태(Empty State)에서도 제어 바(Control Bar)는 **항상 표시**되어야 합니다.
  - 사용자는 가사가 없더라도 발음/번역 토글 버튼 등을 미리 조작하거나 확인할 수 있어야 합니다.
- **Empty State Area**:
  - 제어 바 하단 영역에 "가사가 없습니다" 메시지와 함께 "곡 등록하기" 또는 "로그인" 버튼을 표시하여 사용자의 행동을 유도합니다.
