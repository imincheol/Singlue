...
- **노래방 모드 (Karaoke Mode)**:
  - **진입점**: 플레이어 상단 정보 영역의 '노래방' 토글 버튼.
  - **레이아웃**: 영상 하단에 오버레이되는 형태로, 높이는 영상의 **최대 50%**를 넘지 않습니다.
  - **구성**:
    - **가사 영역**: 유동적 높이. **현재 가사(Current Line)**만 표시하며, 다음 가사는 보여주지 않습니다.
    - **제어 영역**: **고정 높이**. 재생 진행 바(Timeline)와 가사 제어 버튼, 재생 버튼 등이 포함됩니다.
  - **화면 모드**:
