...
- **다국어 맞춤 생성**: 사용자가 선택한 언어(App Language)에 맞춰 발음과 해석을 생성합니다.
  ...
  - **일본어(JA)**: 발음은 가타카나로 표기, 해석은 일본어로 제공.
  - **중국어(ZH)**: 발음은 알파벳(Pinyin) 또는 현지 표기, 해석은 중국어로 제공.
...
- **발음 표기 가독성 강화**: 한국어 가사를 영어나 베트남어 알파벳으로 표기할 때, 단어의 구분을 명확히 하기 위해 음절 사이에 하이픈(`-`)을 추가합니다. (예: "안녕하세요" -> "an-nyeong-ha-se-yo")
- **자동 감지**: 불완전한 가사가 로드되면 "AI로 채우기" 버튼이 자동으로 활성화됩니다.
- **언어별 생성 및 쿼터 (New)**:
  - 기존에는 곡 단위로 쿼터를 차감했으나, 이제는 **언어별(Locale)**로 생성 시마다 쿼터가 1회 차감됩니다.
  - 가사가 존재하지 않는 언어를 선택 시, 사용자에게 생성 여부를 묻는 팝업을 띄웁니다. (회원: 쿼터 차감 / 비회원: 로그인 유도)
...
- **미니 뷰어**: 플레이어 화면 하단에 현재 가사와 다음 가사를 보여주는 뷰어입니다. (노래방 모드 실행 시 숨김 처리됨)
- **노래방 모드 (Karaoke Mode)**:
  - **진입점**: 플레이어 상단 정보 영역의 **우측 끝**에 위치한 '노래방' 토글 버튼.
  - **레이아웃**: 영상 하단에 **Rounded(둥근)** 스타일로 오버레이되는 형태.
  - **구성**:
    - **가사 영역**: **원문/발음/해석** 3단 표시를 지원합니다.
      - **개별 제어**: 각 항목(원문, 발음, 해석)을 켜고 끌 수 있는 토글 버튼을 제공합니다.
      - **폰트 제어**: 가사 크기를 조절할 수 있는 버튼(+/-)을 제공합니다.
    - **제어 영역**: 고정 높이. 재생 진행 바(Timeline)와 재생 버튼 등이 포함됩니다 (종료 버튼 제거).
  - **화면 모드**:
    - **일반 모드**: 영상 가림을 최소화하며 적절한 크기로 표시.
    - **전체화면 모드**: Video Container 전체가 확대되며, 텍스트가 시원시원하게 커집니다(responsive). 제어 영역은 가로로 확장되나 높이는 유지됩니다.

### 🌍 Internationalization (다국어 지원)
- **지원 언어**: 한국어(KO), 영어(EN), 베트남어(VI), 일본어(JA), 중국어(ZH).
- **언어 분리 정책 (Language Separation)**:
  - **사이트 언어 (App Language)**: UI(메뉴, 버튼, 안내 문구)의 언어. 전역 네비게이션(GNB)에서 설정.
  - **노래 언어 (Song Display Language)**: 가사의 발음(Pronunciation)과 해석(Translation)을 표시할 기준 언어. 플레이어 내부의 제어 바에서 별도로 설정.
  - **기본값 전략**: 사용자가 노래에 진입 시, 해당 노래에 '사이트 언어'와 일치하는 데이터가 있다면 자동으로 선택하고, 없다면 원어(Original) 또는 영어를 1순위로 표시합니다.
- **자동 설정**: 사용자의 브라우저 설정에 맞춰 언어가 자동으로 적용됩니다.
- **레이아웃 최적화**: 베트남어 등 문장이 긴 언어에서도 메인 타이틀이 3줄을 넘지 않도록 레이아웃 및 텍스트를 최적화합니다.
- **UI 현지화 (Localization)**:
  - 앱 내 모든 정적 텍스트(메뉴, 버튼, 안내 메시지, 에러 메시지 등)는 선택된 언어(KO, EN, VI, JA, ZH)에 맞춰 번역되어 표시되어야 합니다.
...
- **이메일 인증**: 보안을 위해 가입 시 입력한 이메일로 인증 링크가 발송됩니다. 링크를 클릭하여 인증을 완료해야 로그인이 가능하며, 일반 회원(`user`)으로서 개인 라이브러리 및 노래 등록 기능을 사용할 수 있습니다.
