...
- **UI 현지화 (Localization)**:
  ...
  - 특히 **곡 등록 마법사(Song Registration Wizard)**, **플레이어 컨트롤(Sync, Rate 등)**, **홈 화면 안내 문구** 등 사용자와 상호작용이 많은 부분은 반드시 다국어를 지원해야 합니다.
...
- **관리자 승인제**: (삭제) 신규 가입 시 즉시 로그인이 가능하며 서비스를 이용할 수 있습니다. `pending` 상태에서도 쿼터 내에서 이용이 허용됩니다.
- **역할 관리**: 
  - `admin`: 모든 사용자 관리 및 승인 권한 보유.
  - `admin`: 모든 사용자 관리 및 승인 권한 보유.
  - `user`: 일반 회원. 개인 라이브러리 및 노래 등록 기능 사용 가능. (승인 대기 없이 즉시 이용 가능)
