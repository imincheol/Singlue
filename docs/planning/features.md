...
- **관리자 승인제**: 신규 가입 회원은 `pending` 상태이며, 승인 전까지는 보안을 위해 **자동으로 로그아웃 처리**됩니다. 승인 완료 후부터 정상적인 서비스 이용이 가능합니다.
  - 승인되지 않은 사용자가 로그인을 시도할 경우, 시스템은 해당 계정의 상태를 확인하고 즉시 세션을 종료합니다.
- **역할 관리**: 
  - `admin`: 모든 사용자 관리 및 승인 권한 보유.
  - `user`: 승인된 일반 회원. 개인 라이브러리 및 노래 등록 기능 사용 가능.
